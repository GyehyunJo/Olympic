  ...
  void startPomodoro() {
    ...
    Timer.periodic(Duration(seconds: 1), (Timer timer) {
      if (workDuration > 0 && isWorking) {
        // 작업 시간 중일 때
        workDuration--;
        print("작업 시간: ${workDuration ~/ 60}:${(workDuration % 60).toString().padLeft(2, '0')}");
      } else if (!isWorking && shortBreakDuration > 0) {
        // 휴식 시간 중일 때
        shortBreakDuration--;
        print("휴식 시간: ${shortBreakDuration ~/ 60}:${(shortBreakDuration % 60).toString().padLeft(2, '0')}");
      } else {
        // 세션 종료 및 상태 전환
        if (isWorking) {
